<?php
	...
	if (isset($_GET['x'], $_GET['y']) && filter_var($_GET['x'], FILTER_VALIDATE_EMAIL) && (strlen($_GET['y'] ) == 32)){
		...
		$activation = mysqli_real_escape_string($dbc,$_GET['y']);
		...
		if (mysqli_affected_rows($dbc) == 1){
			...
			echo '<p class="error"> Your account could not be activated, please re-check the  or contact your system admin </p>';
		}
	}
	else{
		//redirect user
